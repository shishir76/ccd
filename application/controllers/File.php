...
class File extends Admin_Core_Controller
{
    ...
    {
       $this->file_model->upload_multiple_files();
        // $images = $this->file_model->get_images($this->file_count);
        // foreach ($images as $image):
        //     echo '<div class="col-sm-2 col-file-manager" id="img_col_id_' . $image->id . '">';
        //     echo '<div class="file-box" data-file-id="' . $image->id . '">';
     ...
     * View Audio File
     */
    public function View_audio_files()
    {
        $results=$this->db->get("audios")->result_array();
        echo json_encode($results);       
    }
    
       public function View_image_files()
    {
        $results=$this->db->get("images")->result_array();
        echo json_encode($results);       
    }
 public function Filter_image_files()
    {  
        $image = trim($this->input->get('image'));
        $this->db->like('image_small',$image);
        $results=$this->db->get("images")->result_array();
        echo json_encode($results);       
    }
     ...
     * View Video File
     */
    public function View_video_files()
    {
        $results=$this->db->get("videos")->result_array();
        echo json_encode($results);
    }
    ...
    public function View_video_details()
    {
        $file_id = $this->input->get('id');
        $this->db->where("id", $file_id);
        $results=$this->db->get("videos")->result_array();
    ...
    public function View_audio_details()
    {
        $file_id = $this->input->get('id');
        $this->db->where("id", $file_id);
        $results=$this->db->get("audios")->result_array();
    ...
    {
        $image_id = $this->input->post('image_ids');
        $audio_id = $this->input->post('audio_ids');
        $video_id = $this->input->post('video_ids');
        
        //delete image
        if($image_id)
        {
            $images_id=explode('~', $image_id);
            $image_count=count($images_id);
            for($i=0; $i< $image_count; $i++)
            {
                $this->file_model->delete_image($images_id[$i]);
            }
        }
        if($audio_id)
        {
            $audios_id=explode('~', $audio_id);
            $audio_count=count($audios_id);
            for($i=0; $i< $audio_count; $i++)
            {
                $this->file_model->delete_audio($audios_id[$i]);
            }
        }
        if($video_id)
        {
            $videos_id=explode('~', $video_id);
            $video_count=count($videos_id);
            for($i=0; $i< $video_count; $i++)
            {
                $this->file_model->delete_video($videos_id[$i]);
            }
        }
        // $file_id = $this->input->post('file_id', true);
        // $this->file_model->delete_video($file_id);
