...
class Post_model extends CI_Model
{
    ...
    //get related custom posts
    public function get_related_custom_posts_limit($category_id, $post_id, $limit, $after_post, $type='related')
    {
        $this->set_filter_query();
        if($type=='related')
        {
        ...
        if($type=='excluded')
        {
            $ex_cat=explode(',', $category_id);
            $this->db->where_not_in('posts.category_id', $ex_cat);
        }
        $this->db->where('posts.id !=', $post_id);
        $this->db->order_by('created_at', 'DESC');
        $this->db->limit($limit,$after_post);
        $query = $this->db->get('posts');
        return $query->result();
    }
  ...
  //get posts by topcategory
   ...
   //get last posts by topcategory
    ...
    //update post flag
    // public function update_post_flag($post)
    // {
    //     if (!empty($post)):
    //         if (!isset($_COOKIE['var_post_' . $post->id])) :
    //             //increase hit
