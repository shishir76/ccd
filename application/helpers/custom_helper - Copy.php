...
if (!function_exists('remove_image_from_html')) {
    function remove_image_from_html($html,$img_src,$text)
    {
        ...
        // $string=preg_replace( '#<[^>]*img[^>]*>#msiU', '', $string );
        // return $string;
...
if (!function_exists('localize_big_image')) {
    ...
    {
        if(strpos($post_image, "http") === false && strpos($post_image, "https") === false)
        {
            $post_image=str_replace('//', 'http://', $post_image); //making url to get image
        }
       
        $post_temp=explode('/', $post_image);
        $basename=$post_temp[count($post_temp)-1];
        $basename_temp=explode('.',$basename);
        $basename=$basename_temp[0].".jpg";
        $basename=str_replace(',', '', $basename); // removing all commas
        $basename=str_replace('\'', '', $basename); // removing all qoutes
        //$rand=date('dmyhis');
        $thumb_w = 750;
        $thumb_h = 422;
        ...
        if (!file_exists($path)) 
        {   
            file_put_contents($path, file_get_contents($post_image));              
        }
